...
#numpy.float64
#pd.DataFrame
with open('C:/Users/jacci/Documents/DS 710/coffee_roasting_profiles/data-raw/saved/Angola--2021-02-25-17-09-27.json') as f:
    data = json.load(f)
# Output: {'name': 'Bob', 'languages': ['English', 'Fench']}
#print(data)
filename = 'C:/Users/jacci/Documents/DS 710/coffee_roasting_profiles/data-raw/saved/Angola--2021-02-25-17-09-27.json'
...
mylist = []
for x in range(0,len(timex[0])):
    mylist.append(game_tweets['timex'][0][x])
timex = tuple(mylist)
#print (mytuple)
mylist = []
for x in range(0,len(t1[0])):
    mylist.append(game_tweets['temp1'][0][x])
t1 = tuple(mylist)

mylist = []
for x in range(0,len(t2[0])):
    mylist.append(game_tweets['temp2'][0][x])
t2 = tuple(mylist)
...
def recomputeDeltas(timex=timex,CHARGEidx = 0 ,DROPidx = 455,t1 = t1,t2=t2,optimalSmoothing=True,timex_lin=None,deltaETsamples=None,deltaBTsamples=None):
    polyfitRoRcalc = True # RIGHT? should be false
    RoRlimitFlag = True
    RoRlimit = 95
    RoRlimitm = -95
# system fixed RoR limits (only applied if flag is True; usually higher than the user configurable once and always applied)
    maxRoRlimit = 170
    deltaBTsamples = int(max(1,10 / 1))
